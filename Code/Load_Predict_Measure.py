...
def predict_func(test_ds,df):
    ...
    # To accomodate both, predict function will always take the second prediction of the softmaxprint(final_model.summary)
    ...
    df['Acc'] = 0
    df.loc[df.results1 == df.Photoshopped, "Acc"] = 1
...
df = predict_func(test_ds_02, df)

y_true = np.array(df['Photoshopped'])
y_pred = np.array(df['results1'])
